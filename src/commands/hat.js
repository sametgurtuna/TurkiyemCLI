...
async function queryAdana(hatNo) {
  ...
  try {
    ...
    const query = hatNo.toUpperCase();
    const matches = listData.buses.filter(b =>
    ...
    spinner.start(`Hat detayları alınıyor (${selectedName})...`);
    const details = await fetchAdanaBusDetails(targetId);
    spinner.succeed(`Hat bilgileri alındı`);
    console.log('');
...
async function querySamsun(hatNo) {
  ...
  try {
    ...
    if (matched.length === 1) {
      ...
      selectedBusName = matched[0].name;
      ...
      selectedBusName = matched.find(b => b.id === selectedLineNo)?.name || '';
    }

    spinner.start('Seçilen hattın saat ve durak bilgileri alınıyor...');

    const details = await fetchSamsunBusSchedule(selectedLineNo);
    if (!details.busName || details.busName.includes('Hat #')) {
      details.busName = selectedBusName || details.busName;
    }
